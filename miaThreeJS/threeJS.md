...
In the command line terminal, run this command inside the folder you installed Three.JS:

```
npm run dev
```

a `localhost` link should appear in the command line. Copy it, and paste it in your browser to view your app. Alternatively, you can hold control(or your alternative Mac key) and left click the generated link to load it

The generated localhost link should look something like this.

![GitBash Local Host](images/screenshots/bashLocalHost.png)
...
Light objects require a **color** and **intensity** for their basic function. For selecting colors, you can use the same RGB hex coes that CSS uses.

Here's a great website that can assist with selecting the color you want!

[https://rgbcolorcode.com/](https://rgbcolorcode.com/)

For now, we shall be using `0xffffff` which is white for our lighting. Next, we will add a comma after establishing the color of the light and pick a number to tell JS what intensity the light will be. Let's pick something simple like 5 to start. 
...
To make your objects appear in the space and move through time, we need to create a new `animate` function and set our animation properties within it.

This code is **required** in order for your ThreeJS script to appear in your localhost.
...
We also added a `roughness` and `metalness` property value to make our new object look more shiny and reflect light. The lower the number for the roughness and metalness the shinnier and more metallic the material becomes. Your scene should look something like this:

![normal mapping](images/screenshots/normalMapping.jpg)

### Structure!

This is for personal convenience sake. 
